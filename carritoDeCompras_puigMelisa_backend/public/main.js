...
socket.on('chats', async (chatInfo) => {
  ...
  const chat = normalizr.denormalize(normalizedChats.result, chatSchema, normalizedChats.entities);
  ...
  const normalizationPercentage = document.getElementById('normalizacion_porcentaje_compresion');
